...
DOMAIN_CERT_ARN=$(aws acm import-certificate \
...
USER_CERT_ARN=$(aws acm import-certificate \
    --certificate fileb://$USER.$DOMAIN.crt \
    --private-key fileb://$USER.$DOMAIN.key \
    --certificate-chain fileb://ca.crt \
    --tags Key=Name,Value=$USER \
...
aws ec2 create-client-vpn-endpoint \
    --client-cidr-block 10.5.0.0/22 \
    --server-certificate-arn $DOMAIN_CERT_ARN \
    --authentication-options Type=certificate-authentication,MutualAuthentication={ClientRootCertificateChainArn=$USER_CERT_ARN} \
    --connection-log-options Enabled=true,CloudwatchLogGroup=clientvpn,CloudwatchLogStream=$DOMAIN/$USER \
    --tag-specifications ResourceType=client-vpn-endpoint,Tags="[{Key=Name,Value=$DOMAIN/$USER}]"
